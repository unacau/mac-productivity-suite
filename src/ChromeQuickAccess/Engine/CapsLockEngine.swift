...
public enum HIDMappingService {
    ...
    public static func applyCapsLockToF18() -> Bool {
        ...
        do {
            try task.run()
            task.waitUntilExit()
            Logger(subsystem: "com.almosteleven.chomyyak", category: "hid").info("Applied Caps Lock -> F18 mapping via hidutil.")
            return task.terminationStatus == 0
        } catch {
            Logger(subsystem: "com.almosteleven.chomyyak", category: "hid").error("Failed to set hidutil mapping: \(error.localizedDescription)")
            return false
        }
    }
    ...
    public static func restoreDefaultMapping() -> Bool {
        ...
        do {
            try task.run()
            task.waitUntilExit()
            Logger(subsystem: "com.almosteleven.chomyyak", category: "hid").info("Restored default HID key mapping.")
            return task.terminationStatus == 0
        } catch {
            Logger(subsystem: "com.almosteleven.chomyyak", category: "hid").error("Failed to restore hidutil mapping: \(error.localizedDescription)")
            return false
        }
    }
}
...
public final class CapsLockEngine: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.almosteleven.chomyyak", category: "engine")
